...
// Normaliza items que vienen del front
...
router.post("/", requireAuth, async (req, res) => {
  try {
    ...
    if (!storeId) {
      return res
        .status(400)
        .json({ message: "storeId es requerido" });
    }
    if (!items.length) {
      return res
        .status(400)
        .json({ message: "items no puede estar vacío" });
    }
